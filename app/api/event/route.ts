...
export async function POST(req: Request) {
  ...
  if (emails.length > 0 && community && organizer) {
    ...
    // Kullanıcı verilerini HTML'e gömerken escape et
    ...
    // TOPLU MAIL SIZINTISI DUZELTMESI: her alici icin ayri mail gonder
    // (Boylece hicbir alici digerlerinin e-postasini goremez)
    await Promise.all(
      emails.map((email) =>
        sendEmail({
          to: [email],
