...
export default function SettingsMenu() {
  ...
  const activeSlug = pathname.split("/").filter(Boolean).pop() ?? "profil";

  return (
    <nav style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {menuItems.map((item) => {
        const isCurrent = item.active && activeSlug === item.slug;
        ...
        if (!item.active) return <div key={item.slug}>{inner}</div>;
        return (
          <Link key={item.slug} href={`/ayarlar/${item.slug}`} style={{ textDecoration: "none" }}>
            {inner}
          </Link>
        );
      })}
